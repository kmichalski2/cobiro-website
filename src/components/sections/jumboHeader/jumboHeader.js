...
const JumboHeader = ( props, { data } ) => {
	console.log(props.sectionContent);
	const textSide = (
		<div className="col col-xs-12 col-md-6 text-left-md">
			<div className={[JumboHeaderStyles.text, 'text-padding'].join(' ')}>
				<h1>{props.sectionContent.heading}</h1>
				<p>{props.sectionContent.text}</p>
				{props.sectionContent.externalLink ? 
					<a href={props.sectionContent.externalLinkUrl}>{props.sectionContent.linkTitle}</a>
					:
					<Link to={props.sectionContent.link.slug} className="btn btn-large space-xs space-sm">{props.sectionContent.linkTitle}</Link>
				}
