...
const BlogPosts = ({ blogPosts, offset, fixedMax, addedAmount, firstLarge, animate, shadow, horizontal, searchTitle, notificationPadding, readLength, topPostButtonText }) => {
    ...
    for (let i = offset || 0; i < (fixedMax || (offset ? numberOfPosts + offset : numberOfPosts)) && i < blogPosts.length; i++) {
        posts.push(
            firstLarge && i === 0 ?
            <BlogPostsHeader key={i} post={blogPosts[i]} searchTitle={searchTitle} search notificationPadding={notificationPadding} readLength={readLength} topPostButtonText={topPostButtonText}/>
            :
